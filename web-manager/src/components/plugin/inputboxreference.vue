<template>
      <el-autocomplete popper-class="my-autocomplete" clearable v-model="name"
                        :fetch-suggestions="queryReference"
                          placeholder="搜索依据"
...
export default {
  ...
  methods: {
    queryReference(queryString, cb) {
      var _this = this;
      var sr = [];
      if (queryString != undefined && queryString.length > 0) {
        axios
          .post(_this.GLOBAL.url_search_reference, {
            sw: queryString
          })
    ...
    selectReference(item) {
      this.name = "";
      this.$emit("selectReference", item);
